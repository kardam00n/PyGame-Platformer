...
class Arrow:
    ...
    def update(self, map):
        pass
    def getV(self, pos, map, time):
        v = ((pos[0] + map.camera.offset.x) ** 2 + (pos[1] + map.camera.offset.y) ** 2) ** 0.5
        if v == 0: v += 0.00001
        v_x = ((pos[0] + map.camera.offset.x) / v) * time * 5
        v_y = ((pos[1] + map.camera.offset.y) / v) * time * 5
        return (v_x, v_y)
class Classic(Arrow):
    def __init__(self, x, y, friendly, pos, time, map):
        ...
        v_x, v_y = super().getV(pos, map, time)
        if v_x<0:
